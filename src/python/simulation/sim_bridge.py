...
def run_simulation():
    ...
    roll_pid = PID(kp=ip_roll['kp'], ki=ip_roll['ki'], kd=ip_roll['kd'], clamp=(-1.0, 1.0))
    ...
    try:
        while True:
            ...
            target_throttle = throttle_ctrl.calculate(
                dz=dz, 
                current_pitch=smooth_desired_pitch, 
                dt=dt
            )
            ...
            drone.set_control_surfaces(target_throttle, pitch_cmd, roll_cmd, yaw_cmd, True)

            # Konsol Çıktısı
            print(f"[{state}] dist={dist_3d:.0f} dz={dz:.0f} hız={drone_speed/27.78:.1f}km/h | "
